...
public class LoggerHelper {
  ...
  public static String hex(byte[] bytes, int offset, int dataLength) {
    ...
    while (pos < dataLength + offset) {
      ...
      if (posHexa == 16) {
        ...
        if (pos + 1 != dataLength + offset) sb.append("| ");
        posHexa = 0;
      }
      pos++;
    }
  ...
  public static String hex(byte[] header, byte[] bytes, int offset, int dataLength) {
    ...
    return hex(complete, 0, dataLength + header.length);
  }
}
