...
public class Statement implements java.sql.Statement {
  ...
  private List<? extends Completion> results = null;
  private Completion completion = null;
  private ExceptionFactory exceptionFactory;
  private List<String> batchQueries = new ArrayList<>();

  public Statement(Connection con, ReentrantLock lock, boolean canUseServerTimeout, int resultSetType,
  ...
  void close(boolean abort) throws SQLException {
    lock.lock();
    try {
      if (!closed) {
        closed = true;
        if (!abort) fetchRemaining();
  ...
  public void setQueryTimeout(int seconds) throws SQLException {
    if (seconds < 0) {
      throw exceptionFactory.create(
          "Query timeout cannot be negative : asked for " + seconds, "42000");
    }
  ...
  public int[] executeBatch() throws SQLException {
    checkNotClosed();
    completion = null;
    if (batchQueries.isEmpty()) return new int[0];
    lock.lock();
    try {
      List<OkPacket> res = con.sendBatch(batchQueries);
      results = res;
      int[] updates = new int[res.size()];
      for (int i = 0; i< res.size(); i++) {
        updates[i] = (int) res.get(i).getAffectedRows();
      }
      completion = results.get(0);
      batchQueries.clear();
      return updates;
  ...
  private String escapeTimeout(final String sql) throws SQLException {
    String escapedSql = escape ? NativeSql.parse(sql, con.getContext()) : sql;
    if (queryTimeout != 0 && canUseServerTimeout) {
      if (maxRows > 0) {
        return "SET STATEMENT max_statement_time=" + queryTimeout + ", SQL_SELECT_LIMIT=" + maxRows + " FOR " + escapedSql;
      }
  ...
  public long executeLargeUpdate(String sql, int autoGeneratedKeys) throws SQLException {
    ...
    return ((OkPacket) completion).getAffectedRows();
  ...
  public long[] executeLargeBatch() throws SQLException {
    checkNotClosed();
    completion = null;
    if (batchQueries.isEmpty()) return new long[0];
    ...
    try {
      ...
      for (int i = 0; i< res.size(); i++) {
        updates[i] = res.get(i).getAffectedRows();
      }
      completion = results.get(0);
      batchQueries.clear();
      return updates;
