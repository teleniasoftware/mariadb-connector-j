...
public class Statement implements java.sql.Statement {
  ...
  private List<Completion> results = null;
  private Completion completion = null;
  private ExceptionFactory exceptionFactory;


  public Statement(Connection con, ReentrantLock lock) {
    this.con = con;
    this.lock = lock;
    this.exceptionFactory = con.getContext().getExceptionFactory().of(this);
  ...
  public ResultSet executeQuery(String sql) throws SQLException {
    ...
    if (results.size() > 0) {
      completion = results.get(0);
      if (completion instanceof Result) return (Result) completion;
    }
    return new CompleteResult(new ColumnDefinitionPacket[0], new ArrayList<>(), exceptionFactory);
  }
   ...
   * object.
   *
   * <p>This limit applies only to <code>BINARY</code>, <code>VARBINARY</code>, <code>LONGVARBINARY
   * </code>, <code>CHAR</code>, <code>VARCHAR</code>, <code>NCHAR</code>, <code>NVARCHAR</code>,
   * <code>LONGNVARCHAR</code> and <code>LONGVARCHAR</code> fields. If the limit is exceeded, the
   * excess data is silently discarded. For maximum portability, use values greater than 256.
   *
  ...
  public int getUpdateCount() throws SQLException {
    checkClose();
    if (completion instanceof OkPacket) {
      return (int) ((OkPacket) completion).getAffectedRows();
    }
    return -1;
  }
   ...
   * @throws SQLException if a database access error occurs, this method is called on a closed
   *     <code>Statement</code> or the given direction is not one of <code>ResultSet.FETCH_FORWARD
   *     </code>, <code>ResultSet.FETCH_REVERSE</code>, or <code>ResultSet.FETCH_UNKNOWN</code>
  ...
  public boolean getMoreResults(int current) throws SQLException {
    ...
    if (completion instanceof ResultSet) {
      lock.lock();
      try {
        Result result = (Result) completion;
        ...
        if (result.streaming()
          ...
          con.readPacket(
              this, results, maxRows, fetchSize, resultSetConcurrency, resultSetScrollType, closeOnCompletion);
    ...
    if (results.size() > 0) {
      completion = results.get(0);
      return (completion instanceof Result);
    }
    completion = null;
    return false;
  }
   ...
   */
  protected void fetchRemaining() throws SQLException {
    if (results.size() > 0) {
      Completion last = results.get(results.size() - 1);

      if (last instanceof ResultSet) {
        Result result = (Result) last;
        result.fetchRemaining();
        if (result.streaming()
            && (con.getContext().getServerStatus() & ServerStatus.MORE_RESULTS_EXISTS) > 0) {
          con.readPacket(this, results, maxRows, 0, resultSetConcurrency, resultSetScrollType, closeOnCompletion);
          while ((con.getContext().getServerStatus() & ServerStatus.MORE_RESULTS_EXISTS) > 0) {
            con.readPacket(this, results, maxRows, 0, resultSetConcurrency, resultSetScrollType, closeOnCompletion);
          }
        }
  ...
  public ResultSet getGeneratedKeys() throws SQLException {
    ...
    if (completion instanceof OkPacket) {
      OkPacket ok = ((OkPacket) completion);
      String lastInsertId = String.valueOf(ok.getLastInsertId());
      return CompleteResult.createResultSet(
          "insert_id", DataType.BIGINT, new String[] {lastInsertId}, exceptionFactory);
    }
  ...
  public int executeUpdate(String sql, int autoGeneratedKeys) throws SQLException {
    ...
    try {
      ...
      results =
          con.sendQuery(
              ...
              fetchSize,
              this.resultSetConcurrency,
              this.resultSetScrollType, closeOnCompletion);
    } finally {
      lock.unlock();
    }
    ...
    if (completion instanceof Result) {
      throw exceptionFactory.create(
          "the given SQL statement produces an unexpected ResultSet object", "HY000");
    }
    return (int) ((OkPacket) completion).getAffectedRows();
  }
  ...
  public <T> T unwrap(Class<T> iface) throws SQLException {
    ...
    throw exceptionFactory.create(
            "The receiver is not a wrapper and does not implement the interface", "42000");
  }
